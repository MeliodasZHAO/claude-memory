...
def _get_default_db_path() -> str:
    """Get the default global database path in the skill directory."""
    # Get the skill directory (parent of scripts directory)
    skill_dir = Path(__file__).parent.parent
    return str(skill_dir / "user-data" / "vector_db")
...
class NoteRetriever:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to ChromaDB database. If None, uses global path in skill directory.
        """
...
def get_relevant_notes(query: str, db_path: Optional[str] = None, top_k: int = 5) -> List[Dict[str, str]]:
    """
    Convenience function to retrieve relevant notes.

    Args:
        query: Query text
        db_path: Path to vector database. If None, uses global path in skill directory.
        top_k: Number of results to return
